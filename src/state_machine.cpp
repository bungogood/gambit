...
State update_state(Chess *chess, int instruction, int side, State state, State_Memory *state_memory, 
    ...
    switch(state){
        ...
        case FriendlyAndEnemyPU:
            ...
            break;
        case InvalidPiecePU:
            if(state_memory->length == 1 && state_memory->memory[0] == instruction){
                state_memory->length --;
                return State::Idle;
            }
            else if (state_memory->length == 2 && is_friendly(chess, side, instruction)){
                state_memory->length --;
                return State::FriendlyPU;
            }
            else if (state_memory->length == 2 && !is_friendly(chess, side, instruction)){
